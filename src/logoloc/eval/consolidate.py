...
def classification_accuracy_given_localized(pairs: list[tuple[float, bool]], iou_threshold: float) -> float:
    localized = [p for p in pairs if p[0] >= iou_threshold]
    if not localized:
        return 0.0
    return sum(1 for _, correct in localized if correct) / len(localized)
...
def build_comparison_table(evaluations: list[ModelEvaluation], primary_iou: float) -> pd.DataFrame:
    ...
    for ev in evaluations:
        ...
        flow2_cls_acc = classification_accuracy_given_localized(ev.flow2.iou_classification_pairs, primary_iou)
        ...
        row = {
            ...
            "impacto_erro_localizacao": flow1_acc - flow2_cls_acc,
        }
