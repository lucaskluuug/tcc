...
O treino é feito no Google Colab pelos notebooks `logoloc_colab.ipynb` (treino) e `logoloc_colab_eval.ipynb` (avaliação, sensibilidade e consolidação). Os notebooks clonam este repositório direto do GitHub. O checkpoint é salvo a cada época, então uma sessão interrompida é retomada de onde parou.
