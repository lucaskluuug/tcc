...
from logoloc.eval.sensitivity import (
    DEFAULT_LEVELS,
    aggregate_sensitivity,
    plot_sensitivity_from_dirs,
    run_sensitivity,
    save_sensitivity,
)
...
def main() -> None:
    ...
    parser.add_argument("--seed", type=int, default=None, help="semente da perturbacao; padrao e a do configs/base.yaml")
    ...
    seed = args.seed if args.seed is not None else project_cfg.seed
    ...
    resultado = run_sensitivity(
        ...
        seed=seed,
        score_threshold=model_cfg.box_score_thresh,
    )
    ...
    logger.info("Semente %d salva em %s", seed, caminho)

    agregado = aggregate_sensitivity(output_dir, args.run_name)
    plot_sensitivity_from_dirs([output_dir], output_dir / "localization_sensitivity.png")
    logger.info("Agregado de %d semente(s) em %s", int(agregado["n_seeds"].iloc[0]), output_dir)
